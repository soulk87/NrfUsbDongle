...
bool adcInit(void)
{
  ...
  err = adc_oneshot_config_channel(adc_h, ADC_CHANNEL_4, &config);
  logPrintf("[%s] adc_oneshot_config_channel()\n", err == ESP_OK ? "OK" : "NG");
  if (err != ESP_OK) return false;

  is_cali = adcCaliInit(ADC_UNIT_1, ADC_ATTEN_DB_12, &adc_cali_h);
...
int32_t adcRead(uint8_t ch)
{
  ...
  lock();
  err = adc_oneshot_read(adc_h, ADC_CHANNEL_4, &adc_raw);
  if (err == ESP_OK)
  {
    if (is_cali) 
    {
